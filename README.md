...
Version **1.3.0** adds a daily update of all four CSVs and an **Update All CSVs**
manual workflow with tests and an optional mode that skips commits and pushes.
Austrian and German Lotto draws remain limited to **2000-01-01 onward**,
inclusive. EuroMillions and Eurojackpot retain their existing histories,
starting in 2004 and 2012 respectively.
...
Version 1.2.0 removed **814 Austrian** and **2,307 German** pre-2000 records; every
...
| `update_all.yml` | Daily at 07:47 UTC, or manual | Run tests, fetch all four archives, validate, and push changed CSVs; retry results published late |
| `_update_lottery.yml` | Called by the individual and all-CSV workflows | Shared fetch, validation, scoped commits, and safe push |
...
The individual schedules keep results current after each drawing. The daily
all-CSV job retries source results that were not available during an earlier
run. Scheduled runs write changed results automatically; a run with no changes
does not create an empty commit. Each all-CSV run executes the offline test
suite before fetching and checks all four archives, including freshness,
before pushing. A failed test, collector, or integrity check stops publication
of that batch.

All update workflows share one writer queue, preventing overlapping pushes
and preserving pending runs. They target the actual default branch; a manual
run selected from a feature branch is skipped. Push retries fetch and rebase,
validate the resulting data again, and never force-push. Same-file conflicts
fail without replacing remote history.

### Run all CSV updates manually

1. Open the repository on GitHub and select **Actions → Update All CSVs**.
2. Click **Run workflow** and select the repository's **default branch**.
3. Leave **push_changes** checked to commit and push updated results.
4. Leave **init** unchecked for a normal update, then click **Run workflow**.

| Input | Default | Behavior |
| --- | --- | --- |
| `push_changes` | `true` | Commit changed canonical CSVs and push them after all checks pass. Set to `false` to fetch and validate in the runner without committing or pushing. |
| `init` | `false` | Set to `true` to revisit each game's supported history: AT/DE from 2000, EuroMillions from 2004, and Eurojackpot from 2012 through the current year. |

The run summary reports changed CSVs and whether the run pushed, found no
changes, ran without pushing, or failed. The four committed archives are
`at/lotto_6aus45/results.csv`, `de/lotto_6aus49/results.csv`,
`eu/euromillions/results.csv`, and `eu/eurojackpot/results.csv`. Public download
copies are regenerated by Pages from those committed files.

A successful run with `push_changes=false` can also trigger the existing
Pages workflow. Pages rebuilds the committed default-branch data; CSV changes
from the test runner are not passed to publication.

The individual collector workflows also support manual runs with `init=true`.
...
2. Under **Settings → Actions → General**, enable the workflows and allow their
   requested `contents: write` permission. If repository or organization policy
   requires it, select **Read and write permissions** under **Workflow permissions**.
   Branch protection or rulesets must permit the configured automation identity
   to update the default branch.
3. Under **Settings → Pages → Build and deployment**, select **GitHub Actions**.
4. Run **Update All CSVs** to test collection and initialize updates. Run
   **Deploy GitHub Pages** if you want to initialize the public download page.

Updates use GitHub's automatically supplied `GITHUB_TOKEN` by default; no
classic token secret is needed for these workflows. Your locally exported
classic token is only used by the upload/push shell script. For an optional GitHub App, set repository
...
app. The original hardcoded app identity has been removed. GitHub documents
[automatic workflow authentication](https://docs.github.com/en/actions/security-for-github-actions/security-guides/automatic-token-authentication)
and the behavior of [scheduled workflows](https://docs.github.com/en/actions/reference/workflows-and-actions/events-that-trigger-workflows#schedule).
Schedules run on the default branch and can be delayed by GitHub. In a public
repository, GitHub can disable schedules after 60 days without repository
activity; re-enable them under Actions if that occurs.
